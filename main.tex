...
\begin{tikzpicture}[remember picture, overlay]
  \node[anchor=north east, inner sep=0pt, xshift=-1.2cm, yshift=-0.6cm]
    at (current page.north east)
    {\includegraphics[height=3cm, keepaspectratio=true]{main.jpg}};
\end{tikzpicture}
...
\begin{itemize}
  ...
  \item 推动多个关键能力上车前实验：单模态 / 多模态轨迹规划、指令变道、OCC 世界模型辅助、GRPO 强化微调。
\end{itemize}
...
\begin{itemize}
  ...
  \item \textbf{多模态轨迹规划}：基于 K-Means 聚类构建 256 锚轨迹库，head 输出 anchor 分类 + 残差回归，top1 指标略优于单模态；在汇入主路、进入辅路、施工绕行、跟车减速刹停等长尾场景输出高质量多模态轨迹。
...
\begin{itemize}
  \item 基于 SIMPL 模型完成业务数据适配，将轨迹预测模型部署上车并持续迭代优化。
  \item 针对他车 cut-in 行为预测不及时的问题，设计自车与他车抢让行交互关系建模的 YOI head，有效提升了该场景下的预测及时性。
\end{itemize}

\section{个人总结}

\begin{itemize}
  \item 具备从 VLM 适配、训练加速、Head 架构设计到指标验证的端到端落地能力，熟悉 Qwen3-VL 系列模型内部机制（M-RoPE、视觉 patch 化、packing 注意力）；关注训练效率与工程化质量，主动推进 packing/flash-attention/混合精度等基础设施优化以缩短迭代周期。
